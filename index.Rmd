...
fit <- lmer(distance ~ (ot1+ot2) + year + ((ot1+ot2) | trial), data = df_individual, REML = FALSE)
summary(fit)
```
...
df_individual %>%
  ...
  stat_summary(fun.data = mean_sdl, geom = 'pointrange', position = position_dodge(0.5)) + 
  stat_summary(fun.y = mean, geom = 'point', size = 2, position = position_dodge(0.5)) + 
