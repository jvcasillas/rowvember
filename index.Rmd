...
```{r, 'total-meters-by-year', results='asis', echo=F}
# Total meters
df_individual %>%
  group_by(rower, year) %>% 
...
### Total meters per week (row time)

```{r, 'total-meters-weekly', echo=F}
...
```{r, 'yearly-improvement-table', echo=F, warning=F, fig.retina=2, fig.width=7, fig.align="center"}
df_individual %>% 
  group_by(., rower, year, time) %>% 
  summarise(., total = sum(distance)) %>% 
  spread(., year, total) %>% 
  ...
  select(., rower, time, starts_with("1")) %>% 
  datatable(.)
...
df_individual %>% 
  ...
  mutate(., `16-17 difference` = `2017` - `2016`, 
            `17-18 difference` = `2018` - `2017`) %>% 
  select(., rower, time, starts_with("1")) %>% 
  gather(., year, diff, -rower, -time) %>% 
  ggplot(., aes(x = year, y = diff, dodge = factor(time), 
                fill = factor(time))) + 
    ...
    labs(y = "Overall distance (m) difference", 
         x = "", title = "Rowvember yearly improvement", 
         subtitle = "Difference between 2016/17 and 2017/18 total distances for 2, 3, 4, and 5 minute rows") + 
    my_theme()
```

### Average meters per round
