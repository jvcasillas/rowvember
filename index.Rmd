...
myfiles = lapply(temp[-c(1:3)], read.csv, sep = ",")
...
df_individual %>%
  group_by(rower, year) %>% 
  summarize(., total_meters = sum(distance), .groups = "drop") %>%
...
time intverval (2, 3, 4, or 5 minutes). 
...
fit <- lme4::lmer(distance ~ (ot1+ot2) + year + 
           ((ot1+ot2) | trial) + 
           ((ot1 + ot2) | year), 
...
df %>%
  ...
  summarise(., totalM = sum(distance), .groups = "drop") %>%
...
df %>%
  ...
  summarise(., totalM = sum(distance), .groups = "drop") %>%
  group_by(., group) %>%
  summarise(., avg = round(mean(totalM), 2), .groups = "drop") %>%
  datatable(.)
```
...
fit_null <- lme4::lmer(distance ~ 1 + (time | athlete) + (trial | athlete), data = df, REML = FALSE)
fit_1    <- lme4::lmer(distance ~ 1 + time + (time | athlete) + (trial | athlete), data = df, REML = FALSE)
fit_2    <- lme4::lmer(distance ~ 1 + time + group + (time | athlete) + (trial | athlete), data = df, REML = FALSE)
fit_full <- lme4::lmer(distance ~ 1 + time * group + (time | athlete) + (trial | athlete), data = df, REML = FALSE)
...
df %>%
  ...
  stat_summary(aes(y = fitted(fit_full)), fun = mean, geom = 'line') + 
